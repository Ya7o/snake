# PATCH 953 — Corrections après audit : Audio feedback

## Contexte
Audit exécuté sur `snake-drive-v4-backup-20260520-2039.tar.gz`. Le projet doit rester mobile-first, en français, avec 8 univers, 16 niveaux, 8 boss et les 24 assets runtime validés.

## Constats principaux
- Signaux audio/mute détectés : 47.
- Signaux feedback visuel/haptique détectés : 89.
- Même sans audio complet, les feedbacks pickup/collision/victoire/défaite doivent être distincts.

## Actions correctives demandées
- [ ] Vérifier feedback pickup, collision, victoire, défaite, boss.
- [ ] Ajouter mute si audio actif.
- [ ] Prévoir feedback visuel si audio désactivé.
- [ ] Éviter sons agressifs ou trop répétitifs.
- [ ] Tester avec volume téléphone bas/muet.

## Contraintes strictes
- Ne pas réintroduire les anciens pipelines assets.
- Ne pas modifier les 24 PNG runtime sans validation explicite.
- Ne pas supprimer les univers/niveaux/boss.
- Ne pas réécrire globalement le jeu.
- Garder la grille Snake prioritaire.
- Mobile Android / Chrome Android prioritaire.

## Tests obligatoires
```bash
npm run check
npm run build
npm run preview -- --host 0.0.0.0
```

## Rapport final obligatoire
Lister :
1. fichiers modifiés ;
2. corrections faites ;
3. tests lancés ;
4. résultat mobile si testé ;
5. risques restants ;
6. blocages / questions.
